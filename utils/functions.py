...
def rotate(img_temp,rot,rot_p):
    if(rot_p>0.5):
        ...
        final_img = final_img[(h_pad-h_inside)/2:(h_pad+h_inside)/2,(w_pad- w_inside)/2:(w_pad+ w_inside)/2,:].astype('uint8')
        return final_img
    else:
        return img_temp
...
def randomizer(img_temp):
    ...
    img_temp = cv2.resize(img_temp,(int(img_temp.shape[1]*ratio),int(img_temp.shape[0]*ratio)))
...
def img_preprocess(img, size=224,augment=False):
