...
def model(image, weights):
    ...
    with tf.name_scope("res4"):
        layers['res4a'] = res_block(layers['res3d'], '4a', weights, stride=2, first=True)
    ...
    with tf.name_scope("res5"):
        layers['res5a'] = res_block(layers['res4f'], '5a', weights, stride=2, first=True)
...
def resnet50(input):
    ...
    net = np.load(global_path + 'resnet50.npy').item()
    weights = {}
