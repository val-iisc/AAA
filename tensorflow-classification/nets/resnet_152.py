...
def model(image, weights):
    ...
    with tf.name_scope("conv1"):
        layers['conv1'] = conv_layer(image, weights['conv1']['weights'], s=2, relu=False)
    ...
    with tf.name_scope("res3"):
        layers['res3a'] = res_block(layers['res2c'], '3a', weights, stride=2, first=True)
    ...
    with tf.name_scope("res4"):
        layers['res4a'] = res_block(layers['res3b7'], '4a', weights, stride=2, first=True)
    ...
    with tf.name_scope("res5"):
        layers['res5a'] = res_block(layers['res4b35'], '5a', weights, stride=2, first=True)
        layers['res5b'] = res_block(layers['res5a'], '5b', weights)
        layers['res5c'] = res_block(layers['res5b'], '5c', weights)

    with tf.name_scope('pool5'):
        layers['pool5'] = tf.nn.avg_pool(layers['res5c'], [1,7,7,1], [1,1,1,1], padding='VALID')
        layers['pool5_r'] = tf.reshape(layers['pool5'],[-1,2048])

    with tf.name_scope('fc1000'):
        layers['fc1000'] = fully_connected(layers['pool5_r'], weights['fc1000']['weights'], weights['fc1000']['biases'])
...
def resnet152(input):
    ...
    global_path = '/data1/krishna/tfff/weights/'
    net = np.load(global_path + 'resnet152.npy').item()
    weights = {}
