...
def save_img_rgb(x, cls_no, iter_):
	x_squeezed = np.squeeze(x)
	temp = x_squeezed - np.min(x_squeezed)
	temp = temp/np.max(temp)
	temp1 = np.copy(temp)
	for i in range(3):
		temp1[:,:,i] = temp[:,:,2-i]
	image_name  = 'proxy_data/ci_c/noise_image_'+net_name+'_' + str(cls_no) + '_' + str(iter_)+ '.png'
	fig = plt.figure(frameon=False)
	ax = plt.Axes(fig, [0.,0.,1.,1.])
	ax.set_axis_off()
	fig.add_axes(ax)
	ax.imshow(temp1, aspect='normal')
...
noise_tv_loss = tf.reduce_mean(tf.image.total_variation(noise_image))


noise_cost = noise_cost_logits + 1e-5*noise_tv_loss
...
saturation = 0
if size != 224: 
	noise_image_base  = resize(np.load('ilsvrc_mean.npy'), (size,size))
else:
...
for cls_no in range(124,1000):
	...
	for sam_no in range(1):
		...
		for j in range(500):
			...
			if j  == 499: 
				train_noise_image, train_noise_loss, train_noise_logits, train_noise_tv = sess.run([noise_image, noise_cost, noise_cost_logits, noise_tv_loss], feed_dict={noise_l: noise_l_train, scale_f: rand_scale_value, rotate_f: rand_rotate_value})
				noise_image_all[cls_no + sam_no, ...] = train_noise_image
				save_img_rgb(train_noise_image, cls_no, j)
				print 'cls', cls_no, 'iter', j, 'pred', np.mean(train_pred), 'confid', np.mean(train_confid), 'evid', np.mean(train_evid), 'n_l', train_noise_loss, 'ci_l', train_noise_logits, 'tv_l', train_noise_tv
				if j == 499:
					break